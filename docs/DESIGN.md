...
4. **Creator's page, not ours:** project branding is a locked masthead wordmark plus a "Create your support page" CTA into GitHub's use-this-template flow, and two tiny footer links — "Powered by buy-me-a-chai" (the repo) and "Support {maker}" (the maintainer's support page). Their identity lives in `src/project.ts` (`MAKER` / `MAKER_PROJECT`); a fork edits two constants or deletes the links (no toggle — the code is public). The links are referral-tagged so clone-driven traffic is traceable to the source project, and an inbound `?ref=`/`?source=` shows a small "Referred via …" chip (ADR-026, ADR-027). They are the only ask of a free project; keep them tiny.
